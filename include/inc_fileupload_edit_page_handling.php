...
if ( isOption($command, $translator->getMessage('MATERIAL_BUTTON_MULTI_UPLOAD_YES')) ) {
   ...
   $focus_element_anchor = 'fileupload';
}

// multi upload
if ( $session->issetValue($file_upload_rubric.'_add_files_multi')
     or $session->issetValue($file_upload_rubric.'_add_files_multi_error')
   ) {
   if ( $session->issetValue($file_upload_rubric.'_multi_upload_post_vars') ) {
      $_POST = $session->getValue($file_upload_rubric.'_multi_upload_post_vars');
      $session->unsetValue($file_upload_rubric.'_multi_upload_post_vars');
   }
   $focus_element_anchor = 'fileupload';
}
...
if ( $session->issetValue($file_upload_rubric.'_add_files_multi') ) {
   ...
   if ( count($file_array_multi) > 0 ) {
      if ( $session->issetValue($file_upload_rubric.'_add_files') ) {
         $file_array = array_merge($session->getValue($file_upload_rubric.'_add_files'),$file_array_multi);
      } else {
         $file_array = $file_array_multi;
      }
      ...
      $new_file_ids = array();
      foreach ($file_array_multi as $key => $temp_array) {
   ...
   $session->unsetValue($file_upload_rubric.'_add_files_multi');
   $from_multiupload = true;
}
...
if ( $session->issetValue($file_upload_rubric.'_add_files_multi_error') ) {
   ...
   $session->unsetValue($file_upload_rubric.'_add_files_multi_error');
   $from_multiupload = true;
}
