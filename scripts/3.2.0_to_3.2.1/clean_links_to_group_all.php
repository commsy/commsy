...
$count_rooms = array_shift(mysql_fetch_row(mysql_query("SELECT COUNT(rooms.item_id) FROM rooms WHERE rooms.deletion_date IS NULL;")));
if ($count_rooms < 1) {
   echo "<br />nothing to do.";
} else {
   init_progress_bar($count_rooms);
   $query  = "SELECT rooms.item_id, rooms.campus_id FROM rooms WHERE rooms.deletion_date IS NULL ORDER BY rooms.item_id;";
   $result = mysql_query($query);
   if ( $error = mysql_error() ) {
   ...
   while ($room_id) {
      ...
      $query  = "SELECT labels.item_id FROM labels WHERE labels.room_id='".$room_id."' AND labels.name='ALL';";
      ...
      $query  = "SELECT user.item_id FROM user WHERE user.room_id='".$room_id."' AND user.deletion_date IS NULL ORDER BY user.item_id;";
      $result2 = mysql_query($query);
      if ( $error = mysql_error() ) {
         echo ('<hr>'.$error.". QUERY: ".$query.'<hr>');
      }
      ...
      while ($user_id) {
         ...
         if (empty($link_id)) {
            ...
            $insert_query = 'INSERT INTO items ( item_id , room_id , campus_id , type , deleter_id , deletion_date , modification_date )
                             VALUES ("", '.$room_id.' , '.$campus_id.', "link_item", NULL , NULL, NULL)';
            ...
            $insert_query = 'INSERT INTO link_items ( item_id, room_id , campus_id , creator_id , deleter_id ,
                             ...
                             VALUES ("'.$link_id.'", "'.$room_id.'","'.$campus_id.'", "'.$user_id.'", NULL , "'
                                       .date("Y-m-d H:i:s").'", NULL, NULL , "'.$user_id.'", '
                                       .'"user", "'.$group_all_id.'", '
                                       .'"group")';
            mysql_query($insert_query);
            if ($error = mysql_error()) {
