...
echo ("This script deletes all old rubric translations in the extra field of all community rooms.");

// count rooms
$count = array_shift(mysql_fetch_row(mysql_query('SELECT COUNT(campus.item_id) FROM campus WHERE extras like "%COURSES_ARRAY%"')));
init_progress_bar($count);

// get all rooms
$query = 'SELECT item_id, extras FROM campus WHERE extras like "%COURSES_ARRAY%"';
