...
$count_project = array_shift(mysql_fetch_row(select("SELECT COUNT(room.item_id) FROM room WHERE extras LIKE '%chat_none%';")));
if ($count_project < 1) {
   ...
   // project projects
   $query  = "SELECT room.item_id,extras FROM room WHERE extras LIKE '%chat_none%' ORDER BY room.item_id;";
   $result = select($query);
   ...
   while ($project_id) {
      ...
      if ($home_conf_exist == true) {
         if (mb_strpos($extra,'chat_none')!=false){
            $set_chat_link = true;
