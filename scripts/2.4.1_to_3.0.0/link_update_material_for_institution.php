...
function updateMaterialForInstitution () {
   global $test, $success, $error;

   $select_query  = "SELECT DISTINCT links.*,
                     labels.item_id, materials.creator_id, materials.creation_date,
                     materials.item_id FROM links LEFT JOIN labels ON links.to_item_id=labels.item_id
                     LEFT JOIN materials ON links.from_item_id = materials.item_id
                     WHERE links.link_type = 'material_for_institution' AND links.from_version_id = materials.version_id AND links.deleter_id IS NULL
   ...
   while ( $row = mysql_fetch_array($result)) {
        ...
        $insert_query = 'INSERT INTO link_items ( item_id , room_id , campus_id , creator_id , deleter_id ,
                          ...
                          VALUES ('.$iid.', '.$row['room_id'].','.$row['campus_id'].', '.$row['creator_id'].', NULL , "'
                              .$row['creation_date'].'", NULL, NULL , '.$row['from_item_id'].', "material", '.$row['to_item_id'].', "institution")';
        $success = select($insert_query);
         if(!$success) {
            echo "<br /><b>mysql complains:</b> ".$error;
            return;
   ...
   $insert_query = 'DELETE FROM links WHERE links.link_type="material_for_institution"';
   $success = select($insert_query);
   if(!$success) {
      echo "<br /><b>mysql complains:</b> ".$error;
      return;
