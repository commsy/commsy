...
function updateMaterialAtCourse () {
   global $test, $success, $error;

   $select_query  = "SELECT DISTINCT links.*,
                     courses.item_id, materials.creator_id, materials.creation_date,
                    materials.item_id FROM links LEFT JOIN courses ON links.to_item_id=courses.item_id
                     ...
                     WHERE links.link_type = 'material_at_courses' AND links.to_version_id = materials.version_id AND links.deleter_id IS NULL
   ...
   while ( $row = mysql_fetch_array($result)) {
        ...
        $insert_query = 'DELETE FROM links WHERE links.link_type="material_at_courses"';
        $success = select($insert_query);
         if(!$success) {
            echo "<br /><b>mysql complains at line ".__LINE__.":</b> ".$error;
            return;
