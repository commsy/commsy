...
function updateMaterialForTopic () {
  global $test, $success, $error;
   ...
   $select_query  = "SELECT DISTINCT links.*,
                     ...
                     WHERE links.link_type = 'material_for_topic' AND links.from_version_id = materials.version_id AND links.deleter_id IS NULL
   ...
   while ( $row = mysql_fetch_array($result)) {
        ...
        $insert_query = 'INSERT INTO link_items ( item_id , room_id , campus_id , creator_id , deleter_id ,
                          ...
                          VALUES ('.$iid.', '.$row['room_id'].','.$row['campus_id'].', '.$row['creator_id'].', NULL , "'
                              .$row['creation_date'].'", NULL, NULL , '.$row['from_item_id'].',"material", '.$row['to_item_id'].', "topic")';
        $success = select($insert_query);
        if(!$success) {
            echo "<br /><b>mysql complains:</b> ".$error;
            return;
   ...
   $insert_query = 'DELETE FROM links WHERE links.link_type="material_for_topic"';
   $success = select($insert_query);
   if(!$success) {
      echo "<br /><b>mysql complains:</b> ".$error;
      return;
