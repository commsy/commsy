...
echo ('This script changes the name of the extrafield MAIL_BODY_USER_STATUS_EDITOR to MAIL_BODY_USER_STATUS_MODERATOR.'."\n");
$success = true;
...
if ($count < 1) {
   ...
   // community rooms
   $query  = "SELECT campus.item_id, campus.extras FROM campus ORDER BY campus.item_id;";
   $result = select($query);
   ...
   while ($room_id) {
      if ( !empty($extra) ) {
         $extra = preg_replace('$<MAIL_BODY_USER_STATUS_EDITOR>$','<MAIL_BODY_USER_STATUS_MODERATOR>',$extra);
         $extra = preg_replace('$</MAIL_BODY_USER_STATUS_EDITOR>$','</MAIL_BODY_USER_STATUS_MODERATOR>',$extra);
      }
