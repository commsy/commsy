...
echo ('This script drops the campus id [take a long time]'."\n");
flush();
...
foreach ($db_tables as $table) {
   ...
   if (in_array('campus_id',$field_array)) {
      $query = 'UPDATE '.$table.' SET room_id = campus_id WHERE room_id = 0 or room_id IS NULL';
      $success = select($query);
      if ($success) {
         $query = 'ALTER TABLE '.$table.' DROP campus_id';
         $success = select($query);
         if ($success) {
            echo(' done'."\n");
...
foreach ($db_tables2 as $table) {
   ...
   if (in_array('cid',$field_array)) {
      $query = 'UPDATE '.$table.' SET rid = cid WHERE rid = 0 or rid IS NULL';
      $success = select($query);
      if ($success) {
         $query = 'ALTER TABLE '.$table.' DROP cid';
         $success = select($query);
         if ($success) {
            echo(' done'."\n");
         }
