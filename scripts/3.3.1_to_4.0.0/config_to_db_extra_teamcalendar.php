...
echo ('This script moves the configuration of the teamcalendar from cs_config.php to db.'."\n");
$success = true;
...
if ($count_rooms < 1) {
   ...
   $extra = $row[1];
   while ($room_id) {
      if ( !empty($teamcalendar_enabled_for) and in_array($room_id,$teamcalendar_enabled_for) ) {
         if ( strstr($extra,'</TEAMCALENDAR>') ) {
            $extra = preg_replace('$<TEAMCALENDAR>0</TEAMCALENDAR>$','<TEAMCALENDAR>1</TEAMCALENDAR>',$extra);
         } elseif ( strstr($extra,'</EXTRA_CONFIG>') ) {
            $extra = preg_replace('$<EXTRA_CONFIG>([\S\s]*)</EXTRA_CONFIG>$','<EXTRA_CONFIG>$1<TEAMCALENDAR>1</TEAMCALENDAR></EXTRA_CONFIG>',$extra);
         } else {
            $extra .= '<EXTRA_CONFIG><TEAMCALENDAR>1</TEAMCALENDAR></EXTRA_CONFIG>';
         }
