...
echo ('This script moves the configuration of the todo-lists from cs_config.php to db.'."\n");
$success = true;
...
if ($count_rooms < 1) {
   ...
   while ($room_id) {
      ...
      if ( !empty($todo_enabled_for) and in_array($room_id,$todo_enabled_for) ) {
         if ( strstr($extra,'</'.CS_TODO_TYPE.'>') ) {
            $extra = preg_replace('$<'.CS_TODO_TYPE.'>0</'.CS_TODO_TYPE.'>$','<'.CS_TODO_TYPE.'>1</'.CS_TODO_TYPE.'>',$extra);
         } elseif ( strstr($extra,'</EXTRA_CONFIG>') ) {
            $extra = preg_replace('$<EXTRA_CONFIG>([\S\s]*)</EXTRA_CONFIG>$','<EXTRA_CONFIG>$1<'.CS_TODO_TYPE.'>1</'.CS_TODO_TYPE.'></EXTRA_CONFIG>',$extra);
         } else {
            $extra .= '<EXTRA_CONFIG><'.CS_TODO_TYPE.'>1</'.CS_TODO_TYPE.'></EXTRA_CONFIG>';
         }
