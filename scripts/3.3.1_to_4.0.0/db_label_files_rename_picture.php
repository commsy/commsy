...
echo ('This script updates file names to the new structure in label items'."\n");
$success = true;

$count = array_shift(mysql_fetch_row(select('SELECT COUNT(item_id) FROM labels;')));
if ($count < 1) {
   echo "<br/>nothing to do.";
} else {
   init_progress_bar($count);

   $query  = 'SELECT item_id,extras FROM labels;';
   $result = select($query);
   $row = mysql_fetch_row($result);
   $id = $row[0];
   $extra = $row[1];
   while ($id) {
      if (strstr($extra,'<LABELPICTURE>')) {
         $change_extra = false;
         $extra_array = xml2array($extra);
         $name = $extra_array['LABELPICTURE'];
         ...
         if ($change_extra) {
            $extra_array['LABELPICTURE'] = $name;
            $extra = array2xml($extra_array);
            $insert_query = 'UPDATE labels SET extras="'.addslashes($extra).'" WHERE item_id="'.$id.'"';
