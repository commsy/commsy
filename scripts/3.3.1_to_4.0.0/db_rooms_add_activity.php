...
echo ('This script adds a table field (activity) to all rooms (projectroom [room] and communityroom [campus]).'."\n");
...
if (!in_array('activity',$field_array)) {
   $query = "ALTER TABLE `rooms` ADD `activity` INT NOT NULL;";
   $success = select($query);
}
unset($field_array);
...
if ($success) {
   ...
   if (!in_array('activity',$field_array)) {
      $query = "ALTER TABLE `campus` ADD `activity` INT NOT NULL;";
      $success = select($query);
   }
}
