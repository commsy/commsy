...
echo ('This script updates file names to the new structure in room items (logo)'."\n");
$success = true;
...
if ($count < 1) {
   ...
   while ($room_id) {
      ...
      if (strstr($extra,'<LOGOFILENAME>')) {
         $change_extra = false;
         $extra_array = xml2array($extra);
         $logo_name = $extra_array['LOGOFILENAME'];
         preg_match('/cid(.+?)_rid(.+?)_(.+)/', $logo_name, $matches);
         if (!empty($matches)) {
  		      if ($matches[2] == '0') {
         ...
         if ($change_extra) {
            $extra_array['LOGOFILENAME'] = $logo_name;
