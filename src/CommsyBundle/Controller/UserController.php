...
class UserController extends Controller
{
    ...
    public function feedActionAction($roomId, Request $request)
    {
        ...
        if ($action == 'markread') {
            ...
            $message = '<i class=\'uk-icon-justify uk-icon-medium uk-icon-check-square-o\'></i> ToDo: '.$translator->transChoice('changed status of %count% users to user',count($selectedIds), array('%count%' => count($selectedIds)));
        } else if ($action == 'user-status-moderator') {
            error_log(print_r('user-status-moderator', true));
            $message = '<i class=\'uk-icon-justify uk-icon-medium uk-icon-check-square-o\'></i> ToDo: '.$translator->transChoice('changed status of %count% users to moderator',count($selectedIds), array('%count%' => count($selectedIds)));
        } else if ($action == 'user-status-blocked') {
            error_log(print_r('user-status-blocked', true));
            $message = '<i class=\'uk-icon-justify uk-icon-medium uk-icon-check-square-o\'></i> ToDo: '.$translator->transChoice('changed status of %count% users to blocked',count($selectedIds), array('%count%' => count($selectedIds)));
        } else if ($action == 'user-status-deleted') {
            error_log(print_r('user-status-deleted', true));
            $message = '<i class=\'uk-icon-justify uk-icon-medium uk-icon-check-square-o\'></i> ToDo: '.$translator->transChoice('changed status of %count% users to deleted',count($selectedIds), array('%count%' => count($selectedIds)));
        }
