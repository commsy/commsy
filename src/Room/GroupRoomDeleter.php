...
/**
 * Deletes a `grouproom`. Replaces `cs_grouproom_item::delete()`.
 * Unlike user / private rooms, a group room has a mirrored label entity
 * (`GROUP_ITEM_ID`), is indexed in ES, and emits moderation mails.
 * User rooms are peers (children of the parent project), not sub-rooms.
 */
class GroupRoomDeleter implements RoomDeleter
{
    ...
    public function softDeleteRoom(int $roomId, int $deleterId, RoomDeletionOptions $opts): void
    {
        ...
        // Mirrored group-as-label row — must die with the room so the
        // group list in the parent project stays consistent.
    ...
    {
        // No extra steps: the mirrored label row is purged by the parent
        // project's hard-delete (it sits in `labels` with context_id = project).
