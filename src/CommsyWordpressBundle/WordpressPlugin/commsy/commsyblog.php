...
class commsy_blog {
   ...
   private function _getSoapClient () {
      ...
      if ( !empty($this->_soap_client) ) {
         ...
         $this->_soap_client = new SoapClient($commsy_auth_commsy_url.'/api/soap?wsdl',$options);
   ...
   public function existsBlog($session_id,$blogId) {
   		error_log(print_r('existsBlog()', true));
      ...
      if ( $this->_isSessionValid($session_id)
         ...
         if ( !$this->_isSessionValid($session_id) ) {
            throw new SoapFault('deleteblog', 'Exists Blog failed: Session-ID ('.$session_id.') is not valid');
         } else {
            throw new SoapFault('deleteblog', 'Exists Blog failed: User ('.$this->_sessionid_to_userid_array[$session_id].') is not allowed to probe for this blog ('.$blogId.').');
         }
      }
      return $retour;
   }
