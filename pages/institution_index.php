...
if ( $mode == 'formattach' or $mode == 'detailattach' ) {
   $attach_type = CS_INSTITUTION_TYPE;
...
if ( !empty($ref_iid) and $mode == 'attached' ){
   $institution_manager->setRefIDLimit($ref_iid);
}
if ( !empty($sort) ) {
   $institution_manager->setSortOrder($sort);
}
if ( !empty($search) ) {
   $institution_manager->setSearchLimit($search);
}
if ( !empty($seltopic) ) {
   $institution_manager->setTopicLimit($seltopic);
}
if ( $interval > 0 ) {
   $institution_manager->setIntervalLimit($from-1,$interval);
}
