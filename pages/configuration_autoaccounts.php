...
function add_user_to_rooms($user, $room_array){
   global $environment;
   $rooms_user_member = array();
   $rooms_user_pending = array();
      ...
      if(isset($room_item)){
         ...
         if(!isset($related_user_item)){
            ...
            if($room_item->checkNewMembersNever()){
               ...
               $rooms_user_member[] = $room;
            } else {
               $user_item->setStatus(1);
               $rooms_user_pending[] = $room;
            }
            $user_item->save();
            //write email to moderators;
            $user_manager = $environment->getUserManager();
            $user_manager->resetLimits();
   ...
   if((!empty($_POST['autoaccount_send_email'])) and ($_POST['autoaccount_send_email'] == 1)){
      include_once('classes/cs_mail.php');
      $mail = new cs_mail();
