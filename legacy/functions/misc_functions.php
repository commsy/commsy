...
function plugin_hook_output_all($hook_function, $params = null, $separator = '', $with_config_check = true, $except = [])
{
    if (!empty($separator)
         and 'ARRAY' == $separator
    ) {
...
function plugin_hook_output($plugin, $hook_function, $params = null, $with_config_check = true)
{
    ...
    if (in_array($plugin, $c_plugin_array)) {
        $do_it = false;
        $plugin_class = $environment->getPluginClass($plugin);
        if ('getSOAPAPIArray' == $hook_function
              or !$with_config_check
        ) {
            $do_it = true;
        } else {
