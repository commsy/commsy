                 ...
                 if ($found > 0) {
                     ...
                     foreach ($matches[0] as $value) {
                         ...
                         foreach ($reg_exp_array as $key => $reg_exp) {
                             ...
                             if ('(:flash' == $key and mb_stristr($value_new, '(:flash')) {
                                 $value_new = $this->_formatFlash($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:wmplayer' == $key and mb_stristr($value_new, '(:wmplayer')) {
                                 $value_new = $this->_formatWmplayer($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:quicktime' == $key and mb_stristr($value_new, '(:quicktime')) {
                                 $value_new = $this->_formatQuicktime($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:image' == $key and mb_stristr($value_new, '(:image')) {
                                 ...
                                 break;
                             } elseif ('(:zip' == $key and mb_stristr($value_new, '(:zip')) {
                                 $value_new = $this->_formatZip($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:youtube' == $key and mb_stristr($value_new, '(:youtube')) {
                                 $value_new = $this->_formatYoutube($value_new, $args_array);
                                 break;
                             } elseif ('(:podcampus' == $key and mb_stristr($value_new, '(:podcampus')) {
                                 $value_new = $this->_formatPodcampus($value_new, $args_array);
                                 break;
                             } elseif ('(:googlevideo' == $key and mb_stristr($value_new, '(:googlevideo')) {
                                 $value_new = $this->_formatGooglevideo($value_new, $args_array);
                                 break;
                             } elseif ('(:vimeo' == $key and mb_stristr($value_new, '(:vimeo')) {
                                 $value_new = $this->_formatVimeo($value_new, $args_array);
                                 break;
                             } elseif ('(:mp3' == $key and mb_stristr($value_new, '(:mp3')) {
                                 $value_new = $this->_formatMP3($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:lecture2go' == $key and mb_stristr($value_new, '(:lecture2go')) {
                                 $value_new = $this->_formatLecture2go($value_new, $args_array);
                                 break;
                             } elseif ('(:slideshare' == $key and mb_stristr($value_new, '(:slideshare')) {
                                 ...
                                 break;
                             } elseif ('(:flickr' == $key and mb_stristr($value_new, '(:flickr')) {
                                 $value_new = $this->_formatFlickr($value_new, $args_array);
                                 break;
                             } elseif ('(:scorm' == $key and mb_stristr($value_new, '(:scorm')) {
                                 $value_new = $this->_formatScorm($value_new, $args_array, $file_array);
                                 break;
                             } elseif ('(:mdo' == $key and mb_stristr($value_new, '(:mdo')) {
                                 $value_new = $this->_formatMDO($value_new, $args_array);
                                 break;
                             } elseif ('(:geogebra' == $key and mb_stristr($value_new, '(:geogebra')) {
...
//            }
    ...
    private function _formatZip($text, $array, $file_name_array)
    {
        ...
        if (!empty($array[1])) {
            if (!empty($file_name_array)
                ...
                if (isset($file)
                     and ('application/x-zip-compressed' == $file->getMime()
                           or 'application/x-compressed' == $file->getMime()
                           or 'application/zip' == $file->getMime()
                           or 'multipart/x-zip' == $file->getMime()
                     )
                ) {
                    ...
                    if ('2' == $file->getHasHTML()) {
                        ...
                        if (!empty($args['target'])
                             and ('_blank' == $args['target']
                                   or 'newwin' == $args['target']
                                   or 'tab' == $args['target']
                             )
                        ) {
                            $display = $args['target'];
    ...
    private function _formatYoutube($text, $array)
    {
        ...
        if (!empty($source)) {
            ...
            if (true) { // only for development, remove after change to new style
                ...
                if ('MSIE' == $this->_environment->getCurrentBrowser()) {
    ...
    public function _formatMP3($text, $array, $file_name_array)
    {
        if ('MSIE' == $this->_environment->getCurrentBrowser()) {
...
//         }
    ...
    public function _formatQuicktime($text, $array, $file_name_array)
    {
        $ext = null;
        $retour = '';
        if (empty($array[1])) {
        ...
        if ('mp3' == mb_strtolower($ext, 'UTF-8')) {
            $args['height'] = 16;
        }
    ...
    private function _formatMDO($value_new, $args_array)
    {
        ...
        if (!empty($args_array[1])) {
            ...
            if ($current_context_item->isProjectRoom()) {
                ...
                if ($community_list->isNotEmpty()) {
                    ...
                    while ($community) {
                        ...
                        if (!empty($mdo_active) && '-1' != $mdo_active) {
            ...
            if (true === $access) {
                ...
                curl_setopt($curl_handler, CURLOPT_POSTFIELDS, ['xmlstatement' => $data]);
                    ...
                    for ($i = 0; $i < $applet_list->length; ++$i) {
                        ...
                        for ($j = 0; $j < $temp_applet_attributes->length; ++$j) {
                            ...
                            if (isset($temp_attribute->nodeName)) {
                                if ('name' == $temp_attribute->nodeName and 'ggbApplet' == $temp_attribute->nodeValue) {
                                    $found_geogebra = true;
                                }
                                if ('codebase' == $temp_attribute->nodeName and './' == $temp_attribute->nodeValue) {
