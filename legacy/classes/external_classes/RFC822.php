...
include_once ('classes/external_classes/PEAR.php');
...
class Mail_RFC822 extends PEAR{
     ...
     */
    function __construct($address = null, $default_domain = null, $nest_groups = null, $validate = null)
    {
    ...
    function splitAddresses($address = '')
    {
        ...
        if ($this->isGroup($address) and !isset($this->error)) {
            $split_char = ';';
            $is_group   = true;
        ...
        if ($is_group and substr($address, 0, 1) == ','){
    ...
    function validateAddress($address)
    {
        ...
        if ($address['group']) {
            ...
            if (!$this->validatePhrase($groupname)){
                $this->error = 'Group name ('.$groupname.') did not validate.';
                return false;
            } else {
    ...
    function validateMailbox($mailbox)
    {
        ...
        if (substr($mailbox, -1) == '>' and substr($mailbox, 0, 1) != '<') {
        ...
        // Only got addr-spec
            ...
            if (substr($mailbox,0,1) == '<' and substr($mailbox,-1) == '>')
        ...
        // If we have a phrase, use that as personal, else use comment
        if (trim($phrase) == '') {
            if (trim($comment) == '') {
                $mbox->personal = '';
            } else {
                $mbox->personal = $comment;
            }
        } else {
            $mbox->personal = $phrase;
            $mbox->comment = $comment;
        }
