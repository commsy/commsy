...
class cs_item_manager extends cs_manager {
  ...
  function cs_item_manager ($environment) {
   ...
   function _performQuery($mode = 'select') {
     if ($mode == 'count') {
        $query = 'SELECT count('.$this->addDatabasePrefix('items').'.id) AS count';
     } elseif ($mode == 'id_array') {
         $query = 'SELECT '.$this->addDatabasePrefix('items').'.id';
     } else {
        $query = 'SELECT '.$this->addDatabasePrefix('items').'.*,label.type AS subtype';
     }
     $query .= ' FROM '.$this->addDatabasePrefix('items');
     $query .= ' LEFT JOIN '.$this->addDatabasePrefix('labels').' AS label ON '.$this->addDatabasePrefix('items').'.id=label.item_id AND (label.type="institution" OR label.type="topic" OR label.type="group")';
     ...
     if ( isset($this->_user_userid_limit) and !empty($this->_user_userid_limit)
        ...
        $query .= ' INNER JOIN '.$this->addDatabasePrefix('user').' ON '.$this->addDatabasePrefix('user').'.parent_id='.$this->addDatabasePrefix($this->_db_table).'.parent_id';
     }
...
/*********************/
     ...
     if ( isset($this->_type_array_limit) and !empty($this->_type_array_limit) ) {
        ...
        foreach($this->_type_array_limit as $type){
           ...
           $query .=' '.$this->addDatabasePrefix('items').'.discriminator = "'.encode(AS_DB,$type).'"';
        }
        $query .= ' )';
     }
     if (isset($this->_room_limit) and empty($this->_room_array_limit)) {
        ...
        $query .= ' AND '.$this->addDatabasePrefix('items').'.parent_id = "'.encode(AS_DB,$this->_environment->getCurrentContextID()).'"';
     }
     if (isset($this->_id_array_limit) and !empty($this->_id_array_limit)) {
        $query .= ' AND '.$this->addDatabasePrefix('items').'.id IN ('.implode(", ",encode(AS_DB,$this->_id_array_limit)).')';
     }
     ...
     if ($this->_delete_limit == true) {
        ...
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
     }
     ...
     if( !empty($this->_room_array_limit)) {
         $query .= ' AND '.$this->addDatabasePrefix($this->_db_table).'.parent_id IN ('.implode(", ",encode(AS_DB,$this->_room_array_limit)).')';
      }
  ...
  function getPrivateRoomItemList ($id_array,$user_ids) {
      ...
      if ( empty($id_array) ) {
         ...
         $query .= ' LEFT JOIN '.$this->addDatabasePrefix('link_modifier_item').' AS modifier ON '.$this->addDatabasePrefix('items').'.id=modifier.item_id';
         $query .=" WHERE ".encode(AS_DB,$this->addDatabasePrefix($type)).".id IN ('".implode("', '",encode(AS_DB,$id_array))."')";
         $query .= ' AND modifier.modifier_id IN ('.implode(",",encode(AS_DB,$user_ids)).')';;
         $query .= ' GROUP BY '.$this->addDatabasePrefix('items').'.id ';
         $query .= ' ORDER BY '.$this->addDatabasePrefix('items').'.modification_date DESC';
         $result = $this->_db_connector->performQuery($query);
         if ( !isset($result) ) {
            include_once('functions/error_functions.php');
   ...
   function getAllUsedRubricsOfRoomList($room_ids){
        $rs = array();
        $query = 'SELECT DISTINCT '.$this->addDatabasePrefix('items').'.parent_id, '.$this->addDatabasePrefix('items').'.discriminator, label.type AS subtype';
        $query .= ' FROM '.$this->addDatabasePrefix('items');
        ...
        $query .= ' WHERE 1';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.parent_id IN ('.implode(",",encode(AS_DB,$room_ids)).')';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "annotation"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "link_item"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "task"';
        if (isset($this->_age_limit)) {
           $query .= ' AND '.$this->addDatabasePrefix('items').'.modification_date > DATE_SUB(CURRENT_DATE,interval '.encode(AS_DB,$this->_age_limit).' day)';
        }
        $query .= ' ORDER BY '.$this->addDatabasePrefix('items').'.parent_id DESC';
        // perform query
        $result = $this->_db_connector->performQuery($query);
        if (!isset($result)) {
   ...
   function getAllPrivateRoomEntriesOfUserList($room_ids,$user_ids){
        ...
        $query .= ' FROM '.$this->addDatabasePrefix('items');
        $query .= ' LEFT JOIN '.$this->addDatabasePrefix('labels').' AS label ON '.$this->addDatabasePrefix('items').'.id=label.item_id';
        $query .= ' LEFT JOIN '.$this->addDatabasePrefix('link_modifier_item').' AS modifier ON '.$this->addDatabasePrefix('items').'.id=modifier.item_id';

       // restrict materials by buzzword (la4)
       if (isset($this->_list_limit)) {
          if ($this->_list_limit == -1){
             $query .= ' LEFT JOIN '.$this->addDatabasePrefix('links').' AS links ON links.from_item_id='.$this->addDatabasePrefix('items').'.id AND links.link_type="in_mylist"';
             $query .= ' LEFT JOIN '.$this->addDatabasePrefix('labels').' AS mylists ON links.to_item_id=mylists.item_id AND mylists.type="mylist"';
          }else{
             $query .= ' INNER JOIN '.$this->addDatabasePrefix('links').' AS links ON links.from_item_id='.$this->addDatabasePrefix('items').'.id AND links.link_type="in_mylist"';
       ...
       if (isset($this->_matrix_limit)) {
          $query .= ' INNER JOIN '.$this->addDatabasePrefix('link_items').' AS matrix_x ON matrix_x.first_item_id='.$this->addDatabasePrefix('items').'.id';
          $query .= ' INNER JOIN '.$this->addDatabasePrefix('link_items').' AS matrix_y ON matrix_y.first_item_id='.$this->addDatabasePrefix('items').'.id';
       }
       ...
       if (isset($this->_buzzword_limit)) {
          if ($this->_buzzword_limit == -1){
             $query .= ' LEFT JOIN '.$this->addDatabasePrefix('links').' AS buzzword_links ON buzzword_links.from_item_id='.$this->addDatabasePrefix('items').'.id AND buzzword_links.link_type="buzzword_for"';
             ...
             $query .= ' INNER JOIN '.$this->addDatabasePrefix('links').' AS buzzword_links ON buzzword_links.from_item_id='.$this->addDatabasePrefix('items').'.id AND buzzword_links.link_type="buzzword_for"';
       ...
       if ( isset($this->_tag_limit) ) {
          ...
          $query .= ' LEFT JOIN '.$this->addDatabasePrefix('link_items').' AS l41 ON ( l41.deletion_date IS NULL AND ((l41.first_item_id='.$this->addDatabasePrefix('items').'.id AND l41.second_item_type="'.CS_TAG_TYPE.'"))) ';
          $query .= ' LEFT JOIN '.$this->addDatabasePrefix('link_items').' AS l42 ON ( l42.deletion_date IS NULL AND ((l42.second_item_id='.$this->addDatabasePrefix('items').'.id AND l42.first_item_type="'.CS_TAG_TYPE.'"))) ';
       }

        if (isset($this->_search_array[0])) {
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('materials').' AS materials ON materials.item_id='.$this->addDatabasePrefix('items').'.id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('todos').' AS todos ON todos.item_id='.$this->addDatabasePrefix('items').'.id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('discussionarticles').' AS discarticles ON discarticles.item_id='.$this->addDatabasePrefix('items').'.id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('dates').' AS dates ON dates.item_id='.$this->addDatabasePrefix('items').'.id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('discussions').' AS discussions ON discussions.item_id='.$this->addDatabasePrefix('items').'.id';
        }
        ...
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "link_item"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "task"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "tag"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "project"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "community"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "grouproom"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "discarticle"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "section"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "step"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "user"';
        $query .= ' AND modifier.modifier_id IN ('.implode(",",encode(AS_DB,$user_ids)).')';;
        if (isset($this->_list_limit)) {
           if ($this->_list_limit ==-1){
   ...
   function getAllNewPrivateRoomEntriesOfRoomList($room_ids){
        $rs = array();
        if ( !empty($room_ids) ) {
           $query = 'SELECT DISTINCT '.$this->addDatabasePrefix('items').'.id, label.type';
           $query .= ' FROM '.$this->addDatabasePrefix('items');
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('labels').' AS label ON '.$this->addDatabasePrefix('items').'.id=label.item_id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('materials').' AS materials ON '.$this->addDatabasePrefix('items').'.id=materials.item_id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('dates').' AS dates ON '.$this->addDatabasePrefix('items').'.id=dates.item_id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('todos').' AS todos ON '.$this->addDatabasePrefix('items').'.id=todos.item_id';
           $query .= ' LEFT JOIN '.$this->addDatabasePrefix('discussions').' AS discussions ON '.$this->addDatabasePrefix('items').'.id=discussions.item_id';
           $query .= ' WHERE 1';
           ...
           $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "step"';
           $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "user"';
           $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "assessments"';
           if (isset($this->_age_limit)) {
              $query .= ' AND '.$this->addDatabasePrefix('items').'.modification_date > DATE_SUB(CURRENT_DATE,interval '.encode(AS_DB,$this->_age_limit).' day)';
           }
   ...
   function getAllNewEntriesOfRoomList($room_ids){
        $rs = array();
        $query = 'SELECT DISTINCT '.$this->addDatabasePrefix('items').'.id';
        $query .= ' FROM '.$this->addDatabasePrefix('items');
        $query .= ' WHERE 1';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.parent_id IN ('.implode(",",encode(AS_DB,$room_ids)).')';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL';
   ...
   function getAllNewEntriesOfHomeView($room_id){
        $rs = array();
        $query = 'SELECT DISTINCT '.$this->addDatabasePrefix('items').'.id, '.$this->addDatabasePrefix('items').'.discriminator, label.type AS subtype';
        $query .= ' FROM '.$this->addDatabasePrefix('items');
        $query .= ' LEFT JOIN '.$this->addDatabasePrefix('labels').' AS label ON '.$this->addDatabasePrefix('items').'.id=label.item_id AND (label.type="institution" OR label.type="topic" OR label.type="group")';
        $query .= ' WHERE 1';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.parent_id ="'.encode(AS_DB,$room_id).'"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "annotation"';
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "discarticle"';
        ...
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "link_item"';
        if (isset($this->_age_limit)){
           $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "announcement"';
           $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "date"';
        }
        $query .= ' AND '.$this->addDatabasePrefix('items').'.discriminator != "task"';
        if (isset($this->_age_limit)) {
           $query .= ' AND '.$this->addDatabasePrefix('items').'.modification_date > DATE_SUB(CURRENT_DATE,interval '.encode(AS_DB,$this->_age_limit).' day)';
        ...
        if (!isset($result)) {
           include_once('functions/error_functions.php');trigger_error('Problems selecting items from query: "'.$query.'"',E_USER_WARNING);
            ...
            foreach ( $result as $query_result ) {
                if ($query_result['discriminator'] == 'label'){
                   $rs[$query_result['subtype']][] = $query_result['id'];
                }else{
                   $rs[$query_result['discriminator']][] = $query_result['id'];
                }
           }
        }
        if (isset($this->_age_limit)){
   ...
   function getCountExistingItemsOfUser ($user_id) {
      $query = 'SELECT count('.$this->addDatabasePrefix('items').'.id) AS count';
      $query .= ' FROM '.$this->addDatabasePrefix('items');
      $query .= ' INNER JOIN '.$this->addDatabasePrefix('link_modifier_item').' AS l1 ON '.$this->addDatabasePrefix('items').'.id=l1.item_id AND l1.modifier_id="'.encode(AS_DB,$user_id).'"';
      $query .= ' WHERE 1';
  ...
  function getItemType($iid) {
      $type = "";
      $query = 'SELECT '.$this->addDatabasePrefix('items').'.discriminator';
      $query .= ' FROM '.$this->addDatabasePrefix('items');
      $query .= ' WHERE id = "'.encode(AS_DB,$iid).'"';
      $result = $this->_db_connector->performQuery($query);
      if ( !isset($result) ) {
         include_once('functions/error_functions.php');
         ...
         foreach ( $result as $query_result ) {
            $type = $query_result['discriminator'];
         }
      }
      return $type;
   }
   ...
   function getExternalViewerEntriesForRoom($room_id) {
      ...
      $query_ids .= ' WHERE parent_id="'.$room_id.'"';
      $result_ids = $this->_db_connector->performQuery($query_ids);
      if ( isset($result_ids) and !empty($result_ids) ) {
      	$id_array = array();
         foreach ( $result_ids as $result_id ) {
            $id_array[] = $result_id['id'].' ';
         }
         $query = 'SELECT id';
         $query .= ' FROM '.$this->addDatabasePrefix('external_viewer');
         $query .= ' WHERE id IN ('.implode(',', $id_array).')';
         $result = $this->_db_connector->performQuery($query);
         if ( isset($result) and !empty($result) ) {
	         foreach ( $result as $query_result ) {
	            $result_array[] .= $query_result['id'];
	         }
         }
      }
      return $result_array;
  ...
  function setCommunityHomeLimit(){
   ...
   public function deleteSpecialItems ($context_id, $type) {
      ...
      $query = 'UPDATE '.$this->addDatabasePrefix($this->_db_table).' SET deleter_id='.encode(AS_DB,$current_user->getItemID()).', deleted_at=NOW() WHERE parent_id='.encode(AS_DB,$context_id).' AND discriminator="'.encode(AS_DB,$type).'"';
      $result = $this->_db_connector->performQuery($query);
   ...
   function deleteReallyOlderThan ($days) {
      ...
      $query = 'DELETE FROM '.$this->addDatabasePrefix($this->_db_table).' WHERE deleted_at IS NOT NULL and deleted_at < "'.$timestamp.'" AND discriminator != "'.CS_DISCARTICLE_TYPE.'" AND type != "'.CS_USER_TYPE.'";'; // user und discarticle werden noch gebraucht
      $result = $this->_db_connector->performQuery($query);
      if ( !isset($result) or !$result ) {
         include_once('functions/error_functions.php');
   ...
   function getCountItems ($start, $end) {
      ...
      $query = "SELECT count(".$this->addDatabasePrefix($this->_db_table).".id) as number FROM ".$this->addDatabasePrefix($this->_db_table)." WHERE ".$this->addDatabasePrefix($this->_db_table).".parent_id = '".encode(AS_DB,$this->_room_limit)."' and ".$this->addDatabasePrefix($this->_db_table).".modification_date > '".encode(AS_DB,$start)."' and ".$this->addDatabasePrefix($this->_db_table).".modification_date < '".encode(AS_DB,$end)."';";
      $result = $this->_db_connector->performQuery($query);
      if ( !isset($result) ) {
         include_once('functions/error_functions.php');
   ...
   function moveFromDbToBackup($context_id){
      ...
      if ( !empty($context_id) ) {
         ...
         if ( !isset($result) ) {
            ...
            $query = 'DELETE FROM '.$this->_db_table.' WHERE '.$this->_db_table.'.id = "'.$context_id.'"';
            $result = $this->_db_connector->performQuery($query);
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
         ...
         $query = 'INSERT INTO '.$c_db_backup_prefix.'_'.$this->_db_table.' SELECT * FROM '.$this->_db_table.' WHERE '.$this->_db_table.'.parent_id = "'.$context_id.'"';
         $result = $this->_db_connector->performQuery($query);
         if ( !isset($result) ) {
            include_once('functions/error_functions.php');
            trigger_error('Problems while copying to backup-table: '.$query,E_USER_WARNING);
         } else {
            $query = 'DELETE FROM '.$this->_db_table.' WHERE '.$this->_db_table.'.parent_id = "'.$context_id.'"';
            $result = $this->_db_connector->performQuery($query);
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
               trigger_error('Problems deleting after move to backup-table: '.$query,E_USER_WARNING);
      ...
      if ( !empty($context_id) ) {
         ...
         if ( !isset($result) ) {
            ...
            trigger_error('Problems while copying from backup-table: '.$query,E_USER_WARNING);
         	...
         	$query = 'DELETE FROM '.$c_db_backup_prefix.'_'.$this->_db_table.' WHERE '.$c_db_backup_prefix.'_'.$this->_db_table.'.id = "'.$context_id.'"';
         	$result = $this->_db_connector->performQuery($query);
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
               trigger_error('Problems deleting after move from backup-table: '.$query,E_USER_WARNING);
            } elseif ( !empty($result[0]) ) {
               $retour = true;
            }
         }
         $query = 'INSERT INTO '.$this->_db_table.' SELECT * FROM '.$c_db_backup_prefix.'_'.$this->_db_table.' WHERE '.$c_db_backup_prefix.'_'.$this->_db_table.'.parent_id = "'.$context_id.'"';
         $result = $this->_db_connector->performQuery($query);
         if ( !isset($result) ) {
            include_once('functions/error_functions.php');
            trigger_error('Problems while copying from backup-table: '.$query,E_USER_WARNING);
         } else {
            $query = 'DELETE FROM '.$c_db_backup_prefix.'_'.$this->_db_table.' WHERE '.$c_db_backup_prefix.'_'.$this->_db_table.'.parent_id = "'.$context_id.'"';
            $result = $this->_db_connector->performQuery($query);
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
   ...
   public function moveFromDbToBackupWorkflow ( $context_id ) {
      ...
      if(!empty($id_array_items) and !empty($id_array_users)){
         if ( !empty($context_id) ) {
            ...
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
               trigger_error('Problems while copying data of "'.$db_table.'" to backup-table: '.$query,E_USER_WARNING);
            } else {
   ...
   function moveFromBackupToDbWorkflow ( $context_id ) {
      ...
      if(!empty($id_array_items) and !empty($id_array_users)){
         if ( !empty($context_id) ) {
         	$query = 'INSERT INTO '.$db_table.' SELECT * FROM '.$c_db_backup_prefix.'_'.$db_table.' WHERE '.$c_db_backup_prefix.'_'.$db_table.'.id IN ('.implode(",", $id_array_items).') OR '.$c_db_backup_prefix.'_'.$db_table.'.user_id IN ('.implode(",", $id_array_users).')';
         	$result = $this->_db_connector->performQuery($query);
            if ( !isset($result) ) {
               include_once('functions/error_functions.php');
               trigger_error('Problems while copying data of "'.$db_table.'" from backup-table: '.$query,E_USER_WARNING);
            } else {
   ...
   private function _deleteFromDbWorkflow($context_id, $from_backup = false){
      ...
      if(!empty($id_array_items) and !empty($id_array_users)){
	      ...
	      if ( !isset($result) ) {
	         include_once('functions/error_functions.php');
	         trigger_error('Problems deleting data of "'.$db_table.'" after move to or from backup-table: '.$query,E_USER_WARNING);
	      } elseif ( !empty($result[0]) ) {
	         $retour = true;
	      }
      }
   ...
   function getItemsForNewsletter ($room_id_array, $user_id_array,$age_limit){
     $query1 = 'SELECT '.$this->addDatabasePrefix('items').'.id, '.$this->addDatabasePrefix('items').'.parent_id, '.$this->addDatabasePrefix('items').'.discriminator FROM '.$this->addDatabasePrefix('items');
     $query1 .= ' WHERE '.$this->addDatabasePrefix('items').'.parent_id IN ('.implode(", ",encode(AS_DB,$room_id_array)).')';
     $query1 .= ' AND modification_date >= DATE_SUB(CURRENT_DATE,interval '.encode(AS_DB,$age_limit).' day) AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL and '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
     $query1 .= ' AND '.$this->addDatabasePrefix('items').'.discriminator !="task" AND '.$this->addDatabasePrefix('items').'.discriminator !="link_item"';
     $query1 .= ' ORDER BY parent_id, discriminator';
     // perform query
     $result1 = array();
     ...
     $query2 = 'SELECT '.$this->addDatabasePrefix('items').'.id,'.$this->addDatabasePrefix('noticed').'.read_date,'.$this->addDatabasePrefix('noticed').'.user_id FROM '.$this->addDatabasePrefix('items');
     $query2 .= ' INNER JOIN '.$this->addDatabasePrefix('noticed').' ON '.$this->addDatabasePrefix('noticed').'.id = '.$this->addDatabasePrefix('items').'.id';
     $query2 .= ' WHERE '.$this->addDatabasePrefix('items').'.parent_id IN ('.implode(", ",encode(AS_DB,$room_id_array)).')';
     ...
     $query2 .= ' AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL and '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
     // perform query
     $r2 = array();
     ...
     $query3 = 'SELECT '.$this->addDatabasePrefix('items').'.id, '.$this->addDatabasePrefix('noticed').'.read_date,'.$this->addDatabasePrefix('noticed').'.user_id FROM '.$this->addDatabasePrefix('items');
     $query3 .= ' INNER JOIN '.$this->addDatabasePrefix('noticed').' ON '.$this->addDatabasePrefix('noticed').'.id = '.$this->addDatabasePrefix('items').'.id';
     $query3 .= ' WHERE '.$this->addDatabasePrefix('items').'.parent_id IN ('.implode(", ",encode(AS_DB,$room_id_array)).')';
     ...
     $query3 .= ' AND '.$this->addDatabasePrefix('items').'.deleter_id IS NULL and '.$this->addDatabasePrefix('items').'.deleted_at IS NULL';
	  #pr($read_date_array);
     #pr($query2);

     $r3 = $this->_db_connector->performQuery($query3);
     foreach($r3 as $r){
     	$read_date_array[$r['user_id']][$r['id']] = $r['read_date'];
     }
     #pr($read_date_array);
     ...
     foreach($result1 as $r){
     	if (!in_array($r['id'],$result2)){
     	   if (isset($r['discriminator']) and $r['discriminator'] == 'annotation'){
     	      $anno_item = $annotation_manager->getItem($r['id']);
     	      ...
     	      unset($anno_item);
     	      unset($linked_item);
     	   }elseif (isset($r['discriminator']) and $r['discriminator'] == 'discarticle'){
     	      $discarticle_item = $discarticle_manager->getItem($r['id']);
      	   ...
      	   unset($discarticle_item);
     	      unset($linked_item);
     	   }elseif (isset($r['discriminator']) and $r['discriminator'] == 'section'){
     	      $section_item = $section_manager->getItem($r['id']);
     	      ...
     	      unset($section_item);
     	      unset($linked_item);
     	   }elseif (isset($r['discriminator']) and $r['discriminator'] == 'step'){
     	      $step_item = $step_manager->getItem($r['id']);
     	      ...
     	      unset($step_item);
     	      unset($linked_item);
     	   }elseif (isset($r['discriminator']) and $r['discriminator'] == 'label'){
     	   	$label_item = $label_manager->getItem($r['id']);
     	      ...
     	      $result[$r['parent_id']][$r['discriminator']][$r['id']][] = 'entry';
     	      $linked_item = $this->getItem($r['id']);

     	      if(empty($read_date_array[$user_id_array[0]][$r['id']])){
     	      	$result[$r['parent_id']][$r['discriminator']][$r['id']]['noticed'] = 'new';
     	      } elseif(isset($read_date_array[$user_id_array[0]][$r['id']])){
     	      	$result[$r['parent_id']][$r['discriminator']][$r['id']]['noticed'] = 'changed';
     	      }
  ...
  function isItemMarkedAsWorkflowRead($item_id, $user_id){
     $query = 'SELECT * FROM '.$this->addDatabasePrefix('workflow_read').' WHERE id = '.$item_id.' and user_id = '.$user_id.';';
     $result = $this->_db_connector->performQuery($query);
     if(empty($result)){
        return false;
     } else {
  ...
  function getUsersMarkedAsWorkflowReadForItem($item_id){
     $result = array();
     $query = 'SELECT * FROM '.$this->addDatabasePrefix('workflow_read').' WHERE id = '.$item_id.';';
     $result = $this->_db_connector->performQuery($query);
     return $result;
  }
  ...
  function markItemAsWorkflowRead($item_id, $user_id){
     if(!$this->isItemMarkedAsWorkflowRead($item_id, $user_id)){
        $query = 'INSERT INTO '.$this->addDatabasePrefix('workflow_read').' (id, user_id) VALUES ('.$item_id.', '.$user_id.');';
        $result = $this->_db_connector->performQuery($query);
     }
  }
  ...
  function markItemAsWorkflowNotRead($item_id, $user_id){
     if($this->isItemMarkedAsWorkflowRead($item_id, $user_id)){
        $query = 'DELETE FROM '.$this->addDatabasePrefix('workflow_read').' WHERE id = '.$item_id.' AND user_id = '.$user_id.';';
        $result = $this->_db_connector->performQuery($query);
     }
  }
  
  function markItemAsWorkflowNotReadForAllUsers($item_id){
     $query = 'DELETE FROM '.$this->addDatabasePrefix('workflow_read').' WHERE id = '.$item_id.';';
     $result = $this->_db_connector->performQuery($query);
  }
}
?>