...
class cs_userroom_manager extends cs_room2_manager
{
    ...
    public function _performQuery($mode = 'select')
    {
        ...
        if (isset($this->_project_room_limit) and !empty($this->_project_room_limit)) {
            ...
            // $query .= ' AND extras LIKE "%<PROJECT_ROOM_ITEM_ID>'.\App\Legacy\SqlStringEscaper::escape($this->_project_room_limit).'</PROJECT_ROOM_ITEM_ID>%"';
            $query .= ' AND extras LIKE "%s:20:\"PROJECT_ROOM_ITEM_ID\";i:'.\App\Legacy\SqlStringEscaper::escape($this->_project_room_limit).';%"';
        }
    ...
    public function getSortedItemList($id_array, $sortBy)
    {
        ...
        if (empty($id_array)) {
            ...
            $query = 'SELECT * FROM '.$this->addDatabasePrefix($this->_db_table).' WHERE '.$this->addDatabasePrefix($this->_db_table).'.item_id IN ("'.implode('", "', \App\Legacy\SqlStringEscaper::escape($id_array)).'") AND '.$this->addDatabasePrefix($this->_db_table).'.type LIKE "'.\App\Legacy\SqlStringEscaper::escape($this->_room_type).'"';
