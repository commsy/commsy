...
{
    public $_average_assessment_id_array = [];
    ...
    public function addAssessmentForItem($item, $assessment)
    {
    ...
    public function getAssessmentForItemAverage($item)
    {
        if (in_array($item->getItemID(), $this->_item_id_array)) {
            if (array_key_exists($item->getItemID(), $this->_average_assessment_id_array)) {
                return [$this->_average_assessment_id_array[$item->getItemID()]['average_assessment'], $this->_average_assessment_id_array[$item->getItemID()]['count_assessment']];
            } else {
                return false;
            }
        } else {
     ...
     public function getAssessmentForItemAverageByIDArray($id_array, $user_id = 0)
     {
         // ------------------
         // --->UTF8 - OK<----
         // ------------------
    ...
    public function getAssessmentForItemDetail($item)
    {
        $query = '
  		SELECT
    ...
    public function hasCurrentUserAlreadyVoted($item)
    {
        $query = '
  		SELECT
  			item_id
    ...
    public function getAssessmentForItemOwn($item)
    {
        $query = '
  		SELECT
  			assessment
