...
class cs_external_page_portal_view extends cs_page_view {
   ...
   function _getWelcomeTextAsHTML () {
      ...
      // NOTE: faculty logo is now displayed in the same row as the portal logo 
   ...
   function asHTML () {
      ...
      if ( !$this->_blank_page ) {
         ...
         if ($this->_show_agbs) {
            ...
            $html .='<tr>'.LF;
            if ( ($left_menue_status != 'disapear' and !$this->_without_left_menue) ) {
               $html .='<td style=" width:13.7em; vertical-align:top;">'.LF;
               $html .= $this->_getLogoAsHTML().LF;
               $html .='</td>'.LF;
              ...
              if ( !empty($logo_filename) and $disc_manager->existsFile($logo_filename) ) {
                 ...
                 unset($params);
                 if ($current_portal->isShowAnnouncementsOnHome()){
                    $html .= '<img class="logo" style="float: right; width:200px;" src="'.$curl.'" alt="'.$this->_translator->getMessage('LOGO').'" border="0"/>';
                 }else{
                    $html .= '<img class="logo" style="float: right; width:300px; height:268px;" src="'.$curl.'" alt="'.$this->_translator->getMessage('LOGO').'" border="0"/>';
                 }
              }
              $html .= '</div>'.LF;
