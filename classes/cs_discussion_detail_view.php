...
class cs_discussion_detail_view extends cs_detail_view {
 ...
 /** array of ids in clipboard*/
    ...
    */
   function cs_discussion_detail_view ($environment, $with_modifying_actions=true,$creatorInfoStatus=array()) {
      $this->cs_detail_view($environment, 'discussion', $with_modifying_actions,$creatorInfoStatus);
   }
   ...
   function _getDiscussionFormAsHTML(){
        if(!(isset($_GET['mode']) and $_GET['mode'] == 'print')) {
         ...
         if ( $discussion_type == 'simple') {
            ...
            $html .= '</td>'.LF;
            $html .= '<td style="width:62%; padding-top:5px; padding-bottom:5px; vertical-align:top; text-align:left;">'.LF;
            ...
            if ( !isset($c_html_textarea)
               ...
               $html .= '<div id="form_formatting_box" style="width:475px">'.LF;
               $html .= $text;
               $html .= '</div>'.LF;
               $html .= '</div>'.LF;
               $html .= '</div>'.LF;
               ...
               $session = $this->_environment->getSessionItem();
                if ($session->issetValue('javascript')) {
                  ...
                  if ($javascript == 1) {
                     ...
                     $html .= '<div id="creator_information'.$current_context->getItemID().'">'.LF;
                     $html .= '<div style="padding:2px;">'.LF;
                     $html .= '<div id="form_formatting_box" style="width:470px">'.LF;
                     $html .= $text;
                     $html .= '</div>'.LF;
                     $html .= '</div>'.LF;
                     $html .= '</div>'.LF;
            ...
            $html .= '<tr>'.LF;
            $html .= '<td class="key" style="border-top:1px solid #B0B0B0; padding-top:10px; vertical-align:top; ">'.LF;
            $html .= '&nbsp;';
            $html .= '</td>'.LF;
            $html .= '<td style="border-top:1px solid #B0B0B0; padding-top:10px; vertical-align:top; white-space:nowrap;">'.LF;
            $html .= '<input name="option" value="'.getMessage('DISCARTICLE_CHANGE_BUTTON').'" tabindex="8" type="submit"/> <input name="option" value="'.getMessage('COMMON_CANCEL_BUTTON').'" tabindex="8" type="submit"/>';
   ...
   function _getDiscussionFormAsHTML2(){
        if(!(isset($_GET['mode']) and $_GET['mode'] == 'print')) {
         $html = '<!-- BEGIN OF DISCARTICLE FORM VIEW -->'.LF.LF;
         $item = $this->getItem();
         $discussion_type = $item->getDiscussionType();
...
#               $html .= '<span style="font-size:small">('.$this->_text_as_html_short($creator_fullname).'&nbsp;'.LF;
#               $html .= $this->_text_as_html_short(getDateTimeInLang($article->getModificationDate())).')</span></td>'.LF;
#               if ($pos_number != 1){
#                  $html .='</div>';
#	            }
            }
   ...
   function _getDetailActionsAsHTML ($item) {
      ...
      $html = '';
      $group_id_list = $item->getLinkedItemIDArray(CS_GROUP_TYPE);
      $groups_attached = empty($group_id_list) ? false : true;
      $current_context = $this->_environment->getCurrentContextItem();
     ...
     if ( $item->isClosed() ) {
         if (!($this->showAllArticles())) {
            ...
            $html .= '> '.ahref_curl( $this->_environment->getCurrentContextID(),
                                      ...
                                      'detail',
                                      $params,
                                      $this->_translator->getMessage('DISCUSSION_CLOSE_ALL_ARTICLES')).BRLF;
            unset($params);
         } else {
            $params = array();
            $params['iid'] = $item->getItemID();
            $html .= '> '.ahref_curl( $this->_environment->getCurrentContextID(),
                                      ...
                                      'detail',
                                      $params,
                                      $this->_translator->getMessage('DISCUSSION_CLOSE_NO_ARTICLES')).BRLF;
            unset($params);
         }
      } else {
         $context_item = $this->_environment->getCurrentContextItem();
         $discussion_type = $item->getDiscussionType();
         if ( $current_user->isUser() and $mod and $this->_with_modifying_actions and $discussion_type == 'simple') {
            $html .= '<a href="#form">> '.$this->_translator->getMessage('DISCARTICLE_ENTER_NEW').'</a>'.BRLF;
         } elseif ($discussion_type == 'simple') {
            $html .= '<span class="disabled">> '.$this->_translator->getMessage('DISCARTICLE_ENTER_NEW').'</span>'.BRLF;
         }
         if ( $item->mayEdit($current_user) and $mod and $this->_with_modifying_actions ) {
            $params = array();
            $params['iid'] = $item->getItemID();
            $html .= '> '.ahref_curl( $this->_environment->getCurrentContextID(),
                                      'discussion',
                                      'close',
                                      $params,
                                      $this->_translator->getMessage('DISCUSSION_CLOSE')).BRLF;
            unset($params);
         } else {
            $html .= '<span class="disabled">> '.$this->_translator->getMessage('DISCUSSION_CLOSE').'</span>'.BRLF;
         }
      }
   ...
   function _getForwardBoxAsHTML () {
      ...
      if ( $current_user->isUser() and $this->_with_modifying_actions ) {
         ...
         $html .= '> '. ahref_curl(  $this->_environment->getCurrentContextID(),
                                    ...
                                    'edit',
                                    $params,
                                    $this->_translator->getMessage('COMMON_NEW_DISCUSSION')).BRLF;
         unset($params);
      } else {
         $html .= '<span class="disabled">'.'> '.$this->_translator->getMessage('COMMON_NEW_DISCUSSION').'</span>'.BRLF;
      }
      $html .= '</div>'.LF;
      $html .= '</div>'.LF;
   ...
   function _getSubItemDetailActionsAsHTML ($subitem) {
      ...
      $html .= '<div class="right_box">'.LF;
/*      $html .= '<div style="float:right; text-align:right; padding: 2px 5px;">';
      ...
      $html .= $this->_getBrowsingIconsAsHTML($subitem, $this->_sub_item_pos_number,$count);
      $html .= '</div>';*/
      $html .= '<div class="right_box_title" style="padding: 2px 5px; font-weight:bold;">'.getMessage('COMMON_ACTIONS').'</div>';
      $html .= '<div class="right_box_main" >'.LF;
      ...
      if ( $user->isUser() and $this->_with_modifying_actions and $discussion_type == 'threaded') {
         ...
         $html .= '> '. ahref_curl(  $this->_environment->getCurrentContextID(),
                                    ...
                                    'edit',
                                    $params,
                                    $this->_translator->getMessage('DISCARTICLE_ANSWER_NEW')).BRLF;
         unset($params);
      } elseif ($discussion_type == 'threaded') {
         $html .= '<span class="disabled">> '.$this->_translator->getMessage('DISCARTICLE_ANSWER_NEW').'</span>'.BRLF;
      }
      if ( $subitem->mayEdit($user) and $this->_with_modifying_actions  ) {
        $params = $this->_environment->getCurrentParameterArray();
        $params['action'] = 'delete';
      ...
      if ( isset($subitems) and !$subitems->isEmpty() ) {
         ...
         while ( $current_item ) {
            ...
            if ($discussion_type == 'threaded'){
               ...
               $html .='<table style="width:71%;" summary="Layout">'.LF;
               $html .='<tr>'.LF;
               ...
               if ($px > 0) {
                  ...
                  $html .='</td>'.LF;
                  $html .='<td style="vertical-align:bottom;">'.LF;
               } else {
                  $html .='<td colspan="2" style="padding-top:5px; vertical-align:bottom;">'.LF;
                  if ( $current_item->isA(CS_DISCARTICLE_TYPE) ) {
                     $html .= '<a id="anchor'.$pos_number.'" name="anchor'.$pos_number.'"></a>'.LF;
                  }
               }
               ...
               $html .= '<h3 class="subitemtitle">'.$this->_getSubItemTitleWithoutNumberAsHTML($current_item);
               $html .= '</h3>'.LF;
               $html .='</div>'.LF;
               $html .='</td>'.LF;
               $html .='</tr>'.LF;
               ...
               $html .='</tr>'.LF;
               $html .='<tr>'.LF;
               $html .='<td style="padding:0px; margin:0px; vertical-align:top;">'.LF;

               $html .='<table style="width:100%; padding:0px; margin:0px;" summary="Layout">'.LF;
               $html .='<tr style="width:100%; padding:0px; margin:0px;">'.LF;
               if ($px >0){
                  $html .='<td style="width:'.$px.'px">&nbsp;'.LF;
                  $html .='</td>'.LF;
               ...
               if(!(isset($_GET['mode']) and $_GET['mode']=='print')){
                  ...
                  $html .='<td style="padding:0px; margin:0px; vertical-align:top;">'.LF;
                  $html .='<table style="border-collapse:collapse; padding:0px; margin:0px;" summary="Layout">'.LF;
                  $html .='<tr style="padding:0px; margin:0px;">'.LF;
                  $html .='<td rowspan="2" style="width:27%; vertical-align:top; padding-left:10px; padding-top:2px;">'.LF;
                  $html .='<div style="margin-bottom:5px;">'.LF;
                  $html .= $this->_getSubItemDetailActionsAsHTML($current_item);
/*               if(!(isset($_GET['mode']) and $_GET['mode']=='print')){
                  $html .='<td colspan="2" style="width:30%; padding-top:5px; padding-left:0px; padding-right:0px; vertical-align:bottom; text-align:right;">'.LF;
                  $html .= $this->_getBrowsingIconsAsHTML($current_item, $pos_number,$count);
                  $html .='</td>'.LF;
               }
*/                  $html .='</div>'.LF;
#                  $html .='<div style="margin-bottom:0px;">'.LF;
#                  $html .= $this->_getAllSubItemLinkedItemsAsHTML($current_item);
#                  $html .='</div>'.LF;
                  $html .='</td>'.LF;
                  $html .='</tr>'.LF;
                  $html .='</table>'.LF;
               ...
               if(!(isset($_GET['mode']) and $_GET['mode']=='print')){
                  ...
                  if ($px >0){
                     $html .='<td style="width:'.$px.'px">&nbsp;'.LF;
                     $html .='</td>'.LF;
                     $html .='<td style="padding-top:0px; vertical-align:top; ">'.LF;
                  }else{
                     $html .='<td colspan="2" style="padding-top:0px; vertical-align:top; ">'.LF;
                  }
                  $html .='<td style="padding-top:5px; vertical-align:top; ">'.LF;
                  $mode = 'short';
                  ...
                  $html .='<tr >'.LF;
                  if ($px >0){
                      $html .='<td colspan="3" style="padding:0px; margin:0px; vertical-align:top;">'.LF;
                     $html .='<table style="width:100%; padding:0px; margin:0px;" summary="Layout">'.LF;
                     $html .='<tr>'.LF;
                     $html .='<td style="width:'.$px.'px; padding-bottom:20px;">&nbsp;'.LF;
                     $html .='</td>'.LF;
                     $html .='<td class="infoborder" style="vertical-align:top; padding-top:0px; padding-bottom:20px; ">'.LF;
                     $html .='</td>'.LF;
                     $html .='</tr>'.LF;
                     $html .='</table>'.LF;
                  }else{
                     $html .='<td class="infoborder" colspan="3" style="width:100%; vertical-align:top; padding-top:0px; padding-bottom:20px; ">'.LF;
                  }
                  $html .='</td>'.LF;
                  $html .='</tr>'.LF;
               }
...
#               $html .='<tr>'.LF;
               ...
               $html .='</td>'.LF;
               if(!(isset($_GET['mode']) and $_GET['mode']=='print')){
                  $html .='<td rowspan="2" style="width:27%; vertical-align:top; padding-top:0px;">'.LF;
                  $html .='<div style="margin-bottom:10px; padding-left:5px;">'.LF;
                  $html .= $this->_getSubItemDetailActionsAsHTML($current_item);
                  $html .='</div>'.LF;
                  $html .='</td>'.LF;
               }
