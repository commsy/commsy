...
/**
 *  class for CommSy list view: label
 */
class cs_labels_index_view extends cs_index_view {

   /** constructor
    * the only available constructor, initial values for internal variables
    *
    * @param object  environment            environment of the page
    * @param string  viewname               e.g. label_index
    * @param boolean with_modifying_actions true: display with modifying functions
    *                                       false: display without modifying functions
    *
    * @author CommSy Development Group
    */
   function cs_labels_index_view ($environment, $with_modifying_actions) {
   ...
   function _getItemTitle($item){
      $title = $item->getTitle();
      return $this->_text_as_html_short($title);
   }
   ...
   function asHTML () {
      ...
      $html .= LF.'<h2 style="margin-bottom:10px; margin-top:0px;">'.LF.
               '   '.$this->_translator->getMessage('LABELS_HEADER').LF.
               '</h2>'.LF;
      $html .= '</div>'.LF;
