...
class cs_group_detail_view extends cs_detail_view {
    ...
    */
   function cs_group_detail_view ($environment, $with_modifying_actions=true,$creatorInfoStatus=array()) {
      $this->cs_detail_view($environment, 'group', $with_modifying_actions,$creatorInfoStatus);
   }
   ...
   function _getDetailActionsAsHTML ($item) {
            ...
            if ( $grouproom_item->isUser($current_user) ) {
               if ( $grouproom_item->isLastModeratorByUserID($current_user->getUserID(),$current_user->getAuthSource()) ) {
                  ...
                  $html .= '> '. ahref_curl( $this->_environment->getCurrentContextID(),
                                             ...
                                             'detail',
                                             $params,
                                             $this->_translator->getMessage('GROUP_LEAVE')).BRLF;
                  unset($params);
               }
            } else {
               ...
               if ( !empty($grouproom_user_item)
                  ...
                  $html .= '<span class="disabled">> '.$this->_translator->getMessage('GROUP_ENTER').'</span>'.BRLF;
               } else {
                  $params = array();
                  $params['iid'] = $this->_item->getItemID();
                  ...
                  $html .= '> '. ahref_curl(  $this->_environment->getCurrentContextID(),
                                             ...
                                             'detail',
                                             $params,
                                             $this->_translator->getMessage('GROUP_ENTER')).BRLF;
                  unset($params);
               }
            }
         }
      ...
      if ( $item->mayEdit($current_user) and !$item->isSystemLabel()) {
