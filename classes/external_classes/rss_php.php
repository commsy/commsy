...
class rss_php {
   ...
   /* changes by Iver Jackewitz */
   /* 08.12.2009 */
   private $_error_internal = false;
...
***/
   private function loadParser($rss=false) {
      if($rss) {
         ...
         /* changes by Iver Jackewitz */
         /* 08.12.2009 */
         if ( $this->_error_internal ) {
            $result = @$DOMDocument->loadXML($rss);
            if ( !empty($result)
                 and $result
               ) {
               $this->_error = false;
   ...
   public function suppressErrors () {
      $this->_error_internal = true;
   }

   public function isRSSOkay () {
      return !$this->_error;
   }
}
?>