      ...
      if(!empty($id_array)){
	      if ( !empty($context_id) ) {
	         ...
	         if ( !isset($result) ) {
	            ...
	            $query = 'DELETE FROM '.$this->addDatabasePrefix($this->_db_table).' WHERE '.$this->addDatabasePrefix($this->_db_table).'.modifier_id IN ('.implode(",", $id_array).')';
	            $result = $this->_db_connector->performQuery($query);
	            if ( !isset($result) ) {
	               include_once('functions/error_functions.php');
      ...
      if(!empty($id_array)){
         if ( !empty($context_id) ) {
            ...
            if ( !isset($result) ) {
               ...
               $query = 'DELETE FROM '.$this->addDatabasePrefix($c_db_backup_prefix.'_'.$this->_db_table).' WHERE '.$this->addDatabasePrefix($c_db_backup_prefix.'_'.$this->_db_table).'.modifier_id IN ('.implode(",", $id_array).')';
               $result = $this->_db_connector->performQuery($query);
               if ( !isset($result) ) {
                  include_once('functions/error_functions.php');
                  trigger_error('Problems deleting after move to backup-table.',E_USER_WARNING);
