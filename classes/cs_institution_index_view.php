...
class cs_institution_index_view extends cs_campus_index_view {
    ...
    * @param string  viewname               e.g. institution_index
    * @param boolean with_modifying_actions true: display with modifying functions
    *                                       false: display without modifying functions
    *
    * @author CommSy Development Group
    */
   function cs_institution_index_view ($environment, $with_modifying_actions) {
   ...
   function _getListActionsAsHTML () {
      ...
      $html .= '<div class="right_box_main" >'.LF;
	   ...
	   if ($current_user->isUser() and $this->_with_modifying_actions ) {
	     $params = array();
	     $params['iid'] = 'NEW';
	     $html .= '> '.ahref_curl($this->_environment->getCurrentContextID(),CS_INSTITUTION_TYPE,'edit',$params,$this->_translator->getMessage('COMMON_NEW_ITEM')).BRLF;
	     unset($params);
	  } else {
     ...
     $params['mode']='print';
     $html .= '> '.ahref_curl($this->_environment->getCurrentContextID(),CS_INSTITUTION_TYPE,'index',$params,$this->_translator->getMessage('COMMON_LIST_PRINTVIEW')).BRLF;
	  $html .= '</div>'.LF;
	  $html .= '</div>'.LF;
