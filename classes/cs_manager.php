...
class cs_manager {
 ...
 /** select items limited by limits
   ...
   */
   ...
   /**
    * implements the general processing of updating search indices, getting the query result from the specific manager
...
protected function updateSearchIndices($query, $item_type = '') {
   	if(!empty($query)) {
		...
		foreach($result as $result_row) {
			...
			$search_data = $result_row['search_data'];
			...
			if(!empty($searchtime_id)) {
				// the search_time id is not null, so this item was already indexed
				// to simplify the process, delete all existing indices in search_index
				$query = '
					DELETE FROM
						search_index
					WHERE
						si_item_id = ' . encode(AS_DB, $item_id);
				$result = $this->_db_connector->performQuery($query);
			}
			...
			foreach($words as $word) {
				...
				// does this word already exists?
				$existing = false;
				foreach($word_result as $word_detail) {
					if($word_detail['sw_word'] == $word) {
						// increase update or set 1 if not set - search_index table
						if(!isset($word_update[$word_detail['sw_id']])) {
							$word_update[$word_detail['sw_id']] = 1;
						} else {
							$word_update[$word_detail['sw_id']]++;
						}
						$existing = true;
						break;
					}
			...
			// perform updates on search_index
			foreach($word_update as $id => $inc) {
				...
				if(empty($result)) {
					...
					$query = '
						INSERT INTO
							search_index(si_sw_id, si_item_id, si_item_type, si_count)
						VALUES
							(' . encode(AS_DB, $id) . ', ' . encode(AS_DB, $item_id) . ', "' . encode(AS_DB, $item_type_tmp) . '", 0)
					';
					$result = $this->_db_connector->performQuery($query);
				}
				...
				$query = '
					UPDATE
						search_index
					SET
						si_count = si_count + ' . encode(AS_DB, $inc) . '
					WHERE
						si_sw_id = ' . encode(AS_DB, $id) . ' AND
						si_item_id = ' . encode(AS_DB, $item_id) . '
				';
