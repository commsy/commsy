 ...
 */
class cs_topic_short_view extends cs_home_view {
    ...
    */
   function cs_topic_short_view ($environment, $with_modifying_actions) {
      $this->cs_home_view($environment, $with_modifying_actions);
      $title = ahref_curl( $this->_environment->getCurrentContextID(),
                           CS_TOPIC_TYPE,
                           'index',
                           '',
                           $this->_translator->getMessage('COMMON_TOPIC_INDEX'),'','','','','','','class="head"');
   ...
   function _getDescriptionAsHTML() {
     ...
     if (isset($list) and $list->isNotEmpty()) {
      ...
      return ' ('.$this->_translator->getMessage('HOME_TOPIC_SHORT_VIEW_DESCRIPTION', $shown).')';
   }
   ...
   function _getContentAsHTML() {
      ...
      if ( !isset($list) || $list->isEmpty() ) {
         $html .= '<tr class="list"><td colspan="2">'.$this->_translator->getMessage('COMMON_NO_TOPICS').'</td></tr>';
      } else {
