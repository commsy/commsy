...
class cs_homepagelink_manager extends cs_manager {
  ...
  function deleteHomepageLinks ($link_id) {
   ...
   public function getFatherItemID ($item_id) {
      $retour = '';
      $query = 'SELECT from_item_id FROM '.$this->_db_table.' WHERE to_item_id="'.encode(AS_DB,$item_id).'";';
      $result = $this->_db_connector->performQuery($query);
      if (!isset($result) or empty($result[0])) {
         include_once('functions/error_functions.php');
   ...
   public function countChildren ($item_id) {
      $retour = 0;
      $query  = 'SELECT count(to_item_id) AS count FROM '.$this->_db_table;
      $query .= ' INNER JOIN homepage_page ON homepage_page.item_id='.$this->_db_table.'.to_item_id';
      $query .= ' WHERE '.$this->_db_table.'.from_item_id="'.encode(AS_DB,$item_id).'" AND homepage_page.page_type="CHILD" AND '.$this->_db_table.'.deleter_id IS NULL AND '.$this->_db_table.'.deletion_date IS NULL;';
      $result = $this->_db_connector->performQuery($query);
      if (!isset($result) or empty($result[0])) {
   ...
   private function _moveUpDown ($direction, $homepage_id, $father_id) {
      $query = 'SELECT sorting_place FROM '.$this->_db_table.' WHERE from_item_id = '.encode(AS_DB,$father_id).' AND to_item_id = '.encode(AS_DB,$homepage_id);
      $result = $this->_db_connector->performQuery($query);
      if (!isset($result) or empty($result[0])) {
         ...
         if ( !empty($result_array['sorting_place']) ) {
            ...
            $query = 'SELECT to_item_id FROM '.$this->_db_table.' WHERE from_item_id = '.encode(AS_DB,$father_id).' AND sorting_place = '.encode(AS_DB,$sorting_place);
            $result = $this->_db_connector->performQuery($query);
            if (!isset($result) or empty($result[0])) {
               ...
               if ( !empty($result_array['to_item_id']) ) {
                  ...
                  $query = 'UPDATE '.$this->_db_table.' SET sorting_place='.encode(AS_DB,$new_sorting_place).' WHERE from_item_id = '.encode(AS_DB,$father_id).' AND to_item_id = '.encode(AS_DB,$to_item_id);
                  $result = $this->_db_connector->performQuery($query);
                  if (!isset($result) or !$result) {
                     ...
                     $query = 'UPDATE '.$this->_db_table.' SET sorting_place='.encode(AS_DB,$new_sorting_place).' WHERE from_item_id = '.encode(AS_DB,$father_id).' AND to_item_id = '.encode(AS_DB,$homepage_id);
                     $result = $this->_db_connector->performQuery($query);
                     if (!isset($result) or !$result) {
                        include_once('functions/error_functions.php');trigger_error('Problems update sortingplace from query: "'.$query.'"',E_USER_WARNING);
                     }
   ...
   private function _cleanSortingPlaces ($item_id) {
      $query = 'SELECT link_id FROM '.$this->_db_table.' INNER JOIN homepage_page ON homepage_page.item_id = '.$this->_db_table.'.to_item_id WHERE '.$this->_db_table.'.from_item_id = '.encode(AS_DB,$item_id).' AND homepage_page.page_type="CHILD" AND '.$this->_db_table.'.deletion_date is NULL AND '.$this->_db_table.'.deleter_id IS NULL ORDER BY sorting_place ASC;';
      $result = $this->_db_connector->performQuery($query);
      $link_id_array = array();
      if (!isset($result)) {
