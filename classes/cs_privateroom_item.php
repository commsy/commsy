...
// Achtung das ist die neue Newsletterfunktion!!!
   function _sendPrivateRoomNewsletter2 () {
      if ( !$this->_send_newsletter ) {
         ...
         $id = $user->getItemID();
         $room_id_array = array();
         $tmp_room_list = $this->getCustomizedRoomList();
         if ( !isset($tmp_room_list) ) {
            $tmp_room_list = $room_manager->_getRelatedContextListForUser($user->getUserID(),$user->getAuthSource(),$portal->getItemID());
         }
         $room_list = new cs_list();
         if ( !$tmp_room_list->isEmpty() ) {
            $item = $tmp_room_list->getFirst();
            while ( $item ) {
               if ( $item->isPrivateRoom()
                   or !$item->isShownInPrivateRoomHomeByItemID($id)
                   or !$item->isOpen()
...
/* Neue fehlerhafte Version Version 1.75
  function _sendPrivateRoomNewsletter () {
      if ( !$this->_send_newsletter ) {
         include_once('functions/misc_functions.php');
         $time_start = getmicrotime();
