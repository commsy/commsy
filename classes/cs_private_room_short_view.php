 ...
 */
class cs_private_room_short_view extends cs_view{
    ...
    */
   function cs_private_room_short_view ($environment,$with_modifying_actions) {
  ...
  /** get the description of the list view title as HTML
   ...
   function _getDescriptionAsHTML() {
      ...
      $description = getMessage('PRIVATE_ROOM_SHORT_VIEW_DESCRIPTION',$time_spread);
   ...
   function _getListInfosAsHTML () {
      ...
      $html  = '';
      $html .= '<div class="right_box" style="margin-top:2px; margin-bottom:1px;">'.LF;
   ...
   function asHTML () {
      ...
      $html .= '<table style="padding-bottom:20px; width: 100%;" summary="Layout">'.LF;
      $context = $this->_environment->getCurrentContextItem();
      $html .= '<tr class="list"><td class="head" colspan="3"><span style="font-weight: bold">'.getMessage('COMMON_ROOM_OVERVIEW').'</span>';
      $html .= ' '.$this->_getDescriptionAsHTML().' </td></tr>'.LF;
      $html .= '<tr>';
         $list = $this->_list;
  ...
  /** get the content of the list view
   ...
   function _getRoomWindowAsHTML ($item) {
      ...
      if ( $this->_with_modifying_actions) {
		  ...
		  $html .= '<img src="images/editdelete_disabled.png" alt="'.$this->_translator->getMessage('LOGO').'" border="0"/>';
	  }
      $html .= '</td>'.LF;
