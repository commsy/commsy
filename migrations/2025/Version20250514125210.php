...
//        $this->addSql('UPDATE room_privat AS rp LEFT JOIN user AS u ON rp.modifier_id = u.item_id SET rp.modifier_id = NULL WHERE rp.modifier_id IS NOT NULL AND (u.item_id IS NULL OR u.account_id IS NULL)');
        ...
        $this->addSql('ALTER TABLE user CHANGE account_id account_id INT DEFAULT NULL, CHANGE item_id item_id INT AUTO_INCREMENT NOT NULL, CHANGE context_id context_id INT NOT NULL, CHANGE creation_date creation_date DATETIME NOT NULL, CHANGE modification_date modification_date DATETIME NOT NULL, CHANGE not_deleted not_deleted TINYINT(1) AS (IF (deleter_id IS NULL AND deletion_date IS NULL, 1, NULL)) PERSISTENT AFTER deletion_date, CHANGE status status SMALLINT NOT NULL, CHANGE visible visible TINYINT(1) NOT NULL, CHANGE extras extras LONGTEXT DEFAULT NULL COMMENT \'(DC2Type:array)\'');
