...
*/
   ...
   function getRoomItemAsHTML($item) {
      ...
      $html .='<div class="info-link-h1">Raumbeschreibung</div>'.LF;
      ...
      if (empty($mode)){
         ...
         $html .= '</td>'.LF;
         $html .= '<td style="width:49%; vertical-align:middle; padding:0px; margin:0px;">'.LF;
   ...
   public function _getIncludedCSSIE5AsHTML () {
      return $this->_getIncludedCSSAsHTML();
   }
   ...
   function _getIncludedCSSAsHTML(){
      ...
      $retour .= '<style type="text/css" media="all">'.LF;
      $retour .= '#startseite div.panel-row-1 div.panel-col-1 { width: 99%; }'.LF;
      $retour .= '#startseite div.panel-row-2 div.panel-col-1 { width: 69.5%; }'.LF;
      $retour .= '#startseite div.panel-row-2 div.panel-col-2 { width: 29.5%; }'.LF;
      $retour .= '#startseite div.panel-row-3 div.panel-col-1 { width: 99%; }'.LF;
      $retour .= '</style>'.LF;
      ...
      $retour .= '   <link rel="stylesheet" media="screen" type="text/css" href="css/external_portal_styles/'.$this->_environment->getCurrentContextID().'/life.css"/>'.LF;
   ...
   function getSearchBoxAsHTML(){
      ...
      #}
      $html .= '   <select style="width: '.$width.'; font-size:10pt; margin:5px 0px 0px 0px;" name="selroom" size="1" onChange="javascript:document.indexform.submit()">'.LF;
...
/*
      if ( $current_user->isRoot() or $current_user->isModerator() ) {
         $html .= '      <option value="8"';
...
/*
      ...
      $html .= '<div style="text-align:left; font-size: 8pt; font-weight:normal; margin:5px 0px 10px 0px;">';
...
*/
      ...
      $html .= '<div style="text-align:left; font-size: 8pt; font-weight:normal; margin-bottom:10px; margin-top:3px;">'.LF;
      ...
      if ( $this->_environment->inPortal()
         ...
         $html .= '   <select style="width: '.$width.'; font-size: 10pt; margin:5px 0px 10px 0px;" name="seltime" size="1" onChange="javascript:document.indexform.submit()">'.LF;
         $html .= '      <option value="-3"';
      ...
      $html .= '<div><input style="margin:5px 0px 10px 0px; font-size:10pt;" name="option" value="'.$this->_translator->getMessage('COMMON_SHOW_BUTTON').'" type="submit"/></div>'.LF;
      $html .= '</div>'.LF;
      $html .= '</div>'.LF;
      $html .= '</div>'.LF;
      $html .= '</form>'.LF;
...
*/
   ...
   function _getUserPersonalAreaAsHTML () {
      ...
      $retour .= '         <select size="1" style="font-size:10pt; width:155px;" name="room_id" onChange="javascript:document.room_change.submit()">'.LF;
      $context_array = array();
...
*/
   ...
   function getMyAreaAsHTML() {
      ...
      $html .= '<div>'.LF;
      $html .= '<div class="info-link-h1">'.LF;
      ...
      if ( $this->_with_personal_area and empty($cs_mod)) {
         if ( !empty($this->_current_user) and ($this->_current_user->getUserID() == 'guest' and $this->_current_user->isGuest()) and !$this->_environment->inServer() ) {
            ...
            $html .= '</td></tr>'.LF.'<tr><td style="padding:0px;margin:0px;" >'.LF;
            ...
            if ( !$this->_environment->inServer() ) {
               ...
               // AGB link
               $html .= '<span style="font-size:10pt;">&gt; '.ahref_curl($this->_environment->getCurrentContextID(),'agb','index','',$this->_translator->getMessage('COMMON_AGB_CONFIRMATION_LINK_INPUT'),'','','','','','','style="display:inline;"').'</span>'.BRLF;
            ...
            if (!$this->_environment->inServer()) {
               ...
               if ( !$current_user_item->isRoot() ) {
                  $html .= '<div class="myarea_section_title" style="padding-top:5px; border-bottom:0px;">'.$this->_translator->getMessage('MYAREA_MY_ACTUAL_ROOMS').'</div>'.LF;
...
/*
            if (!$this->_environment->inServer() ) {
               ...
               if ( !$this->_current_user->isRoot()
                  ...
                  $html .= '<div class="myarea_section_title">'.$this->_translator->getMessage('MYAREA_MY_PROFILE').' (CommSy)</div>'.LF;
...
#        $html .= '</div>'.LF;
   ...
   function AnnouncementsAsHTML ($announcement_view) {
      ...
      $retour .= '<table id="cs_portal_announcements" summary="Layout">'.LF;
      ...
      if ( isset($announcement_view->_list) and !$announcement_view->_list->isEmpty() ) {
         ...
         if($community->isOpenForGuests() OR $community->isUser($this->_environment->getCurrentUser()) OR $this->_environment->getCurrentUser()->isRoot()) {
               ...
               if ( !empty($logo) ) {
                  $text .= '        <td class="first-column">'.LF;
                  $text .= $logo.LF;
                  $text .= '        </td>'.LF;
                  $text .= '        <td class="second-column">'.LF;
               } else {
                  $text .= '        <td colspan="2" class="one-column">'.LF;
               }
               $current_user = $this->_environment->getCurrentUserItem();
               ...
               $text .= '   </tr>'.LF;
               $text .= '   <tr>'.LF;
               $text .= '       <td colspan="3" class="second-row">'.LF;
   ...
   function _getPortalAnnouncements(){
      ...
      $html .= '<div>';
      $html .= '<div id="p_announcements" style="margin:0px; padding:0px 0px;">'."\n";
   ...
   function getContentListAsHTML(){
      ...
      if ( isset($this->_room_list_view) ){
         ...
         $html .='</tr>'.LF;
         $html .='<tr>'.LF;
         $html .='<td colspan="3" style="vertical-align:bottom; font-size:8pt;">'.LF;
...
#         $html .='</td>'.LF;
 ...
 #        $html .='<table style="width:100%; margin:0px; padding:0px;" summary="Layout">'.LF;
         ...
         $html .= '<table style="width: 500px; border-collapse: collapse; border: 0px; padding:0px;" summary="Layout">'.LF;
   ...
   function asHTML(){
     ...
     */
     ...
     /********Konfiguration********/
      ...
      $html .= '<div id="alles">
                   <div id="header">
                      <div id="logos"><a class="logolinks" href="http://www.uni-hamburg.de/"></a><a class="logorechts" href="http://www.epb.uni-hamburg.de/"></a></div>
                      <div id="bildcontainer">
                         <div class="bild">
                            <img src="http://life.epb.uni-hamburg.de/files/header/raeder.jpg" alt="Headerbild"/>
                         </div>
                         <div id="brotkasten">
                            <div class="schattenlinks">
                               <div class="schattenrechts">
                                  <div class="breadcrumb1">           <a href="http://life.epb.uni-hamburg.de/"><span id="lifelogo">
     ...
     $html .= '    <div id="hintergrund">
                      <div id="inhalt">
                         <div class="schattenlinks">
                            <div class="schattenrechts">
                               <div id="menu1">
                                  <ul class="linkslife" id="navlistlife">
                                     <li><a href="http://life.epb.uni-hamburg.de/'.$add_param.'">START</a></li>
                                     <li><a href="http://life.epb.uni-hamburg.de/neuigkeiten'.$add_param.'">NEUIGKEITEN</a></li>
                                     <li><a href="http://life.epb.uni-hamburg.de/veranstaltungen'.$add_param.'">VERANSTALTUNGEN</a></li>
                                     <!-- <li><a href="http://life.epb.uni-hamburg.de/portraits'.$add_param.'" class="menu-1-3-2">PORTRAITS</a></li> -->
                                     <li><a href="http://life.epb.uni-hamburg.de/uniblogs'.$add_param.'" class="menu-1-4-2">UNIblogs</a></li>
                                     <!-- <li><a href="http://life.epb.uni-hamburg.de/node/84'.$add_param.'" class="menu-1-5-2">DOSSIERS</a></li> -->
                                     <!-- <li><a href="http://life.epb.uni-hamburg.de/studentisches'.$add_param.'" class="menu-1-6-2">COMMUNITY</a></li> -->
                                     <li><a href="'.curl($this->_environment->getCurrentContextID(),'home','index',array()).'" class="menu-1-7-2-active">EDUCommSy</a></li>
                                  </ul>
                               </div>'.LF;


     $html .= '                <div id="sidebar-left" class="sidebar" style="margin-top:0px; padding:0px;">
                                  <div id="block-user-0" class="clear-block block block-user">
                                     <div class="content" style="background-color:transparent; border:0px;">'.LF;
     ...
     $html .= '               <div class="center">
                                 <div class="ansicht">
                                    <div class="content" style="padding-top:0px; border:0px;">'.LF;

      $html .= '<table style="background-color:#FFFFFF; padding:0px; margin:0px;">'.LF;
      $html .='<tr>'.LF;
      $html .= '<td>'.LF;
      $html .='<img style="width:200px;" src="css/external_portal_styles/'.$current_context->getItemID().'/educommsy.jpg" title="EduCommSy"/>'.LF;
      $html .= '</td>'.LF;
      $html .= '</tr>'.LF;
      $html .= '<tr>'.LF;
      $show_list = true;
      if ( isset($this->_agb_view) or
         ...
         $html .= '<td style="100%; vertical-align:top;">'.LF;
      }else{
         $html .= '<td style="width:520px; vertical-align:top;">'.LF;
      }
      if ( isset($this->_agb_view) ) {
         $html .= $this->_getAGBViewAsHTML().LF;
      }
     ...
     if ( !(isset($this->_agb_view) or
          ...
          $html .= '<td style="width:200px; float:right; vertical-align:top;">'.LF;
          if ( isset($this->_room_list_view) ){
             $current_portal_item = $this->_environment->getCurrentPortalItem();
             if ( $current_portal_item->isShowAnnouncementsOnHome() ) {
                $html .= '<div id="portal_announcements" style="margin-bottom:15px; padding:0px; margin-left:0px margin-right:0px; width:190px;">'.LF;
                $html .= '<div class="info-link-h1">Ankündigungen</div>'.LF;
                $html .= $this->_getPortalAnnouncements();
                $html .= '</div>'.LF;
             }
             unset($current_portal_item);

             $html .= '<div id="portal_search" style="margin-bottom:15px; padding:0px; margin-left:0px margin-right:0px; width:190px;">'.LF;
             #$html .= '<div class="info-link-h1" style="width:185px;">Raumsuche</div>'.LF;
             $html .= '<div class="info-link-h1">Raumsuche</div>'.LF;
             $html .= $this->getSearchBoxAsHTML().LF;
             $current_user = $this->_environment->getCurrentUser();


             if ($current_user->isUser()){
                $html .= '<div id="portal_action" style="margin-bottom:15px; padding:0px; margin-left:0px margin-right:0px; width:190px;">'.LF;
                $html .='<div>'.LF;
                $html .='<div class="info-link-h1">Aktionen</div>'.LF;
            ...
            if ( $current_context->showServiceLink()
               ...
               if ( !empty($email_to_service) ) {
                ...
                if ( $current_context->withAGB() and !isset($this->_agb_view) and $this->_with_agb_link ) {
                   include_once('functions/curl_functions.php');
                   $html .= BRLF;
                   $html .= '> '.ahref_curl($this->_environment->getCurrentContextID(),'agb','index','',$this->_translator->getMessage('COMMON_AGB_CONFIRMATION_LINK_INPUT'));
                }
