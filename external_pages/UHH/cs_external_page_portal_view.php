...
*/
  ...
  return $html;
   ...
   function getRoomItemAsHTML($item) {
      ...
      $html .= '<div id="room_detail_head"></div>'.LF;
    ...
    if ( !$this->_environment->inPortal() || $this->_current_user->isGuest() ) {
          ...
          if ( $this->_with_personal_area and empty($cs_mod)) {
             if ( !empty($this->_current_user) and ($this->_current_user->getUserID() == 'guest' and $this->_current_user->isGuest()) and !$this->_environment->inServer() ) {
                ...
                // login form
                $html .= '<table summary="Layout" style="margin:10px">'.LF;
...
//                      }
                ...
                if (!$this->_environment->inServer()) {
                   ...
                   if ((!$user->isRoot() and $user->isUser()) or ($user->isGuest() and $user->getUserID() != 'guest')
                      ...
                      if ( isset($own_room)
                         ...
                         if ($link_active) {
                            ...
                            $html .= ahref_curl($own_room->getItemID(), 'home', 'index', '',$this->_translator->getMessage('MYAREA_LOGIN_TO_OWN_ROOM'),'','','','','','','').'</span>'.BRLF;
                         } else {
                   ...
                   if ( $this->_current_user->isRoot() ) {
                      ...
                      $html .= ahref_curl($this->_environment->getServerID(), 'home', 'index', '',$this->_translator->getMessage('MYAREA_LOGIN_TO_ALL_PORTALS'),'','','','','','','').'</span>'.LF;
                   }
                } else {
                   if ( $this->_current_user->isRoot() ) {
                      $html .= '<div class="myarea_content">'.LF;
...
#         $html .= '<h2>'.$this->_translator->getMessage('PORTAL_ENTER_ROOM').'</h2>'.LF;
      ...
      elseif ($cs_room_id and $cs_module == 'configuration' ){
        ...
        $html .= $this->getDetailWelcomeBox();
      }

      if ($show_list){
        $html .= '<div id="room_list_all">';
