...
class Mail extends PEAR
{
    ...
    function factory($driver, $params = array())
    {
        ...
        @include_once 'classes/external_classes/'.$driver . '.php';
        $class = 'Mail_' . $driver;
        if (class_exists($class)) {
//          "return new $class($params);" the original generation of the object by horde.org
//          changed by Oliver Hankel, CommSy
            return new $class();
        } else {
    ...
    function prepareHeaders($headers)
    {
        ...
        foreach ($headers as $key => $val) {
            if ($key == 'From') {
                include_once 'classes/external_classes/RFC822.php';

                $from_arr = Mail_RFC822::parseAddressList($val, 'localhost', false, false);
        ...
        $retour = array($from, $text_headers);
        return $retour;
    }
    ...
    function parseRecipients($recipients)
    {
        ...
        $addresses = Mail_RFC822::parseAddressList($recipients, 'localhost', false, false);
        $recipients = array();
