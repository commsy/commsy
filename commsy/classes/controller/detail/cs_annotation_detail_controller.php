<?php
	...
	class cs_annotation_detail_controller extends cs_detail_controller {
		/**
		 * constructor
		 */
		public function __construct(cs_environment $environment) {
			...
			$this->_tpl_file = 'annotation_detail';
		}
		...
		public function processTemplate() {
			...
			$this->assign('room', 'rubric', CS_ANNOTATION_TYPE);
		}
      ...
      $page->add($errorbox);
					...
					if($session->issetValue('annotation_clipboard')) {
						$clipboard_id_array = $session->getValue('annotation_clipboard');
					}

					// copy to clipboard
					if(isset($_GET['add_to_annotation_clipboard']) && !in_array($current_item_id, $clipboard_id_array)) {
						$clipboard_id_array[] = $current_item_id;
						$session->setValue('annotation_clipboard', $clipboard_id_array);
					}
					...
					if($session->issetValue('cid' . $this->_environment->getCurrentContextID() . '_annotation_index_ids')) {
						$annotation_ids = $session->getValue('cid' . $this->_environment->getCurrentContextID() . '_annotation_index_ids');
					}
		...
		protected function setBrowseIDs() {
			...
			if($session->issetValue('cid' . $this->_environment->getCurrentContextID() . '_annotation_index_ids')) {
				$this->_browse_ids = array_values((array) $session->getValue('cid' . $this->_environment->getCurrentContextID() . '_annotation_index_ids'));
			}
		}
