...
function collectFiles ( $dir, $file_array ) {
   ...
   while ( false !== ($entry = readdir($directory_handle)) ) {
      if ( $entry != '.'
         ...
         $regex = 'cid'.$dir_name.'_[0-9]*_';
         if ( is_numeric($dir_name)
              and strstr($entry,'cid'.$dir_name.'_')
              and mb_ereg($regex,$entry)
            ) {
...
foreach ( $file_array as $file ) {
   $values = array();
   if ( preg_match('#cid([0-9]*)_([0-9]*)_#',$file,$values) ) {
      if ( !empty($values[1]) and !empty($values[2]) ) {
         $fileid = $values[2];
         $cid = $values[1];

         $sql = 'SELECT context_id FROM files WHERE files_id="'.$fileid.'";';
         $result = select($sql);
         unset($sql);
         ...
         if ( !$row ) {
            ...
            $filename2 = str_replace(basename($file),$fileid,$file);
