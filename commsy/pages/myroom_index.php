      ...
      switch ($_POST['index_view_action']) {
         case 1:
            ...
            foreach ($selected_ids as $id) {
               ...
               $reader_manager->markRead($id, $version_id );
               ...
               if ( !empty($annotation_list) ){
                  ...
                  while($annotation_item){
                     $reader_manager->markRead($annotation_item->getItemID(),'0');
         ...
         case 2:
            ...
            break;
         case 3:
            $action = 'ENTRY_MAKE_INACTIVE_ROOM';
            $error = false;
            $room_manager = $environment->getRoomManager();
            $reader_manager = $environment->getReaderManager();
            foreach ($selected_ids as $id) {
         ...
         case 4:
           ...
           if( $user->isModerator() ){
               ...
               $page->addDeleteBox(curl($environment->getCurrentContextID(),'myroom','index',$params),'index',$selected_ids);
               unset($params);
            }
            break;
         default:
