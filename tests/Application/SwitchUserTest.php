 ...
 *
 * The unit-level rule is pinned in
 * {@see \Tests\Integration\Security\Voter\SwitchToUserVoterTest}. These tests
 * answer the question that voter test cannot: does the parameter actually
...
final class SwitchUserTest extends AbstractApplicationTestCase
{
     ...
     *
     * `getCanImpersonateAnotherUser()` is an opt-OUT
    ...
    public function testPlainMemberCannotImpersonateAnotherMember(): void
    {
        ...
        self::assertSame(
            $actor->getUsername(),
            $this->authenticatedUsername(),
            'a plain member must keep their own identity — the opt-out default is not a grant',
        );
    }

    /**
     * The severe case. UserProvider::loadUserByIdentifier() special-cases the
     * identifier 'root' and returns the server-context root account BEFORE any
     * portal scoping, so the portal boundary that covers every other account
     * does not reach it. The decision rests entirely on SwitchToUserVoter,
     * hence this case is pinned on its own.
     */
    ...
    /**
     * Not even a portal moderator may become root: root has no portal, so the
     * voter's portal comparison refuses it.
     */
